...
def _generate_pattern_info(img: np.ndarray, opts: PreProcOpts, 
    ...
    if opts.friedel_refine and (peak_data['nPeaks'] >= opts.min_peaks):  
        ...
        if (pkl.shape[0] >= opts.min_peaks): #after the filtering the friedel radius, the number of peaks can be too small
            ctr_refined, cost, _ = _ctr_from_pks(pkl, np.array([x0, y0]), int_weight=opts.int_weight, 
...
def _get_corr_img(img: np.ndarray, 
    ...
    if opts.remove_background:
        img = remove_background(img, x0, y0, nPeaks, peakXPosRaw, peakYPosRaw, pxmask=None, detector = opts.detector, peak_radius=opts.peak_radius, filter_len=opts.filter_len,interp=True)
